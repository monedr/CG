...
int main(void)
{
	...
	int nbFrames    = 0;
	//tempo usado para animação do objeto 
	double lastAniTime = glfwGetTime();
	...
	do{
		...
		// Desenha segundo objeto
		
		// As operações a seguir fazem o com que o segunfo objeto faz uma rotação ao redor do primeiro sempre "olhado" para o primeiro
		// A linha abaixo realiza a rotação ao redor do primeiro obejto
		ModelMatrix = glm::rotate(ModelMatrix, (float)(((currentTime - lastAniTime) * 360) / 5), glm::vec3(0, 1, 0));
		// Esta linha move o segudo objeto da origem e vira ele de forma q ele fique "olhando" o primeiro completando a operação
		ModelMatrix = glm::translate(ModelMatrix, glm::vec3(4, 0, 0)) * glm::rotate(glm::mat4(1.0), glm::degrees(-90.0f), glm::vec3(0,1,0));
		...
		if ((currentTime - lastAniTime) > 5) {
			lastAniTime = glfwGetTime();
		}
